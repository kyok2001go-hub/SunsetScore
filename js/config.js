...
(function (root) {
  ...
  root.SunsetScore.config = {
    ...
    /* ---------- Spatial Cloud Field（升级方案 二~八 章） ---------- */
    spatialField: {
    ...
    /* ---------- Spatial Cloud Field 增强（增强方案 四~八 章） ---------- */
    spatialEvolution: {
      /* Cloud Continuity：沿距离链的相邻相似度（方案 4.2 节） */
      continuity: { midWeight: 0.4, highWeight: 0.6 },
      /* Spatial Gradient：type 判定阈值 [TUNE]（方案 4.3 节） */
      gradient: { farType: 20, nearType: -20 },
      ...
      skyCanvasWeights: { local: 0.30, bank: 0.30, far: 0.15, structure: 0.15, antiSunset: 0.10 }
    },
    ...
    /* ---------- Weather Regime 动态权重（技术方案 五~十五 章） ---------- */
    weatherRegime: {
    ...
    forecastWindow: { lookbackHours: 6, lookaheadHours: 1, nowLookbackHours: 8, nowLookaheadHours: 2 },

    /* ---------- Nowcasting 临近预报（技术方案 Phase 1-3） ---------- */
    nowcast: {
      enabled: true,
    ...
    evolution: {
      ...
      probClamp: [0.02, 0.98],
      /* 唯一背景先验混合权重；其余来源走概率模型，不维护失效五源权重。 */
      backgroundWeight: 0.30,
    ...
    /* ---------- 日落事件层与 Golden Window V4（技术方案 第 7、8 章） ---------- */
    goldenWindow: {
      enabled: true,
      beforeSunsetMinutes: 180,
