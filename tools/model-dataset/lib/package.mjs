...
export function contents(result) {
  ...
  return { 'schema.json': canonicalJson(modelSchema(version)), 'policy.json': canonicalJson(modelPolicy(result.policyVersion || version)),
...
export function makeManifest(source, result, files, createdAt = new Date().toISOString()) {
  ...
  const descriptor = { model_dataset_schema_version: version, model_dataset_policy_version: result.policyVersion || version,
  ...
  return { model_dataset_schema_version: version, model_dataset_policy_version: result.policyVersion || version, ...identity(descriptor), ...source,
