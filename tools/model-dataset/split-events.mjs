...
export function splitEvents(events, onBoundary = () => {}) {
  ...
  for (let i = 1; i < D - 1; i++) for (let j = i + 1; j < D; j++) {
    ...
    if (sizes.every((n, k) => n >= POLICY.minimum_events[k]) && (!selected || objective < selected.objective)) selected = { i, j, sizes, objective };
  }
  ...
  const blockers = [];
  if (N < POLICY.minimum_events.reduce((a, b) => a + b, 0)) blockers.push('TOO_FEW_PRIMARY_EVENTS');
  ...
  return { status: selected ? 'READY' : 'INSUFFICIENT_SPLIT_DATA', publishable: !!selected,
    ...
    date_blocks: blocks, minimum_total_event_deficit: Math.max(0, 50 - N), minimum_date_block_deficit: Math.max(0, 3 - D),
