...
本工具处理 Phase 1–6 的正式离线发布包，提供两项能力：

1. `dataset:lineage`：只读枚举六阶段发布包，按每个包的完整主 ID 生成依赖图。
...
| Phase 6 Parameter Optimization | `dataset/optimization/exports/` | `optimization_id` | schema 1 + policy 1 |
...
自定义 `--output` 构建的包只有在该根目录被显式登记后才会被扫描；默认契约只覆盖上表六个根目录。

## 2. 检测

```powershell
# 默认：六阶段包数、每个包的完整主 ID、诊断与 Mermaid 图
...
Mermaid 默认输出**扁平图**：只有节点声明与连线，不使用 `subgraph`。分栏方框更好看，但跨 subgraph 的连线是部分内嵌渲染器会静默丢弃的写法，因此需要时再传 `--group`。节点的 Phase 前缀（`raw_` / `gt_` / `model_` / `baseline_` / `sensitivity_` / `optimization_`）本身就能区分阶段。

图节点只写完整主 ID；路径、Hash、文件数与字节数放在清单与 JSON 里。边分三类，全部参与级联闭包：

```text
SOURCE     上游是计算结果输入（Raw→GT、Raw/GT→Model、Model→Evaluation、Model/Evaluation→Sensitivity、Model/Evaluation/Sensitivity→Optimization）
REFERENCE  Sensitivity / Optimization 重复声明但没有直接 Hash 的 Raw / GT 引用，会与 Model 声明交叉核对
EVIDENCE   Sensitivity / Optimization 绑定的验证集披露证据
```

`EVIDENCE` 是最容易漏掉的边：删除披露证据包时，引用它的 Sensitivity 和 Optimization 也必须进入清理闭包。

绘图只保留包之间的**直接关系**。Phase 1–6 本身按顺序推进，因此凡是目标已经能通过其他包到达的边，都属于重复表述，不画在图上：Sensitivity / Optimization 重复声明的 Raw / GT（`REFERENCE`）、以及已经能通过中间阶段到达的 `SOURCE` 边。规则是对当前边集做传递归约，逐个检查“删掉这条边后目标是否仍可达”，因此**可达性完全不变**，压缩后的图仍然能回答“删一个包会带走谁”。

JSON 的 `edges` 始终是完整集合，另附 `display_edges` 与 `drawing` 计数；清理计划同样使用完整集合，级联范围不受绘图压缩影响。
...
# 执行：解析闭包 → 写计划 → 按 Phase 6→1 删除
npm run dataset:prune -- baseline_v1_7e48bb52a4b9_626d7073a563

# Phase 6 包也可直接预览；只会选择该 Optimization 包及其未来下游
npm run dataset:prune -- optimization_v1_fbce6cdefa41_81e312bdcd71 --dry-run
```

不接受前缀、通配符、日期区间或“整阶段删除”，也不接受一次多个 ID。计划文件自动写到 `dataset/maintenance/plans/<完整主ID>.json`，作为这次删除的持久记录；重复执行同一 ID 会覆盖这份记录。可选加 `--phase <1-6>` 作为断言，阶段不符时报 `PHASE_MISMATCH`。

从目标沿全部反向依赖求可达闭包，按 Phase 6→1、同阶段主 ID 字符码排序执行。输出包含每个目标的 manifest SHA-256、路径、文件数、字节数、入选原因与依赖边，并醒目列出证据边。
...
逐包处理顺序为 Phase 6→1、同阶段按主 ID 字符码。每个包先原子重命名到 `dataset/maintenance/quarantine/<plan_sha256>/phase<N>/<主ID>`，写 `operations.log.jsonl` 的 `MOVED`，再删除隔离目录并写 `DELETED`。发布根目录只会看到包“完整存在”或“不存在”。中断恢复不承诺跨包整体原子性，但始终下游先行。

## 5. 维护门禁

六个构建器（Raw / GT / Model / Evaluation / Tuning / Optimization）在整个构建期间持有共享租约 `dataset/maintenance/leases/<uuid>/`：
...
2026-09-22 本地 Phase 1–6 共 12 个正式包（`2/2/2/2/2/2`），依赖图 26 条声明边、0 条诊断，绘图压缩为 10 条。Phase 6 的两个 Optimization 包都正确关联到相应的 Model、Evaluation、Sensitivity、Raw 与 GT；删除任一上游包时，它们会进入下游清理闭包。
